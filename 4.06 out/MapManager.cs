...
namespace AdminPanel
{
    public class MapManager
    {
        ...
        public static Map ReadMap(MapType mapType)
        {
            ...
            //чтение из файла
            var SR = new StreamReader(@"../maps/" + fileName);
            var textFromFile = SR.ReadToEnd();

            //разбиение полученной строки на массив символов
            var mapContext = textFromFile.Replace("\r", "").Split('\n');

            var mapHeight = mapContext[0].Length;
            var mapWidth = mapContext.Count();

            var map = new Map();
            var mCells = new List<List<CellMapType>>();

            for (var x = 0; x < mapHeight; x++)
            {
                mCells[x] = new List<CellMapType>();

                for (var y = 0; y < mapWidth; y++)
                {
                    if (mapContext[x][y] == 'c' || mapContext[x][y] == 'с')
                    {
                        mCells[x].Add(CellMapType.Wall);
                    }
                    else if (mapContext[x][y] == 'т')
                    {
                        mCells[x].Add(CellMapType.Grass);
                    }
                    else if (mapContext[x][y] == 'в')
                    {
                        mCells[x].Add(CellMapType.Water);
                    }
                    else if (mapContext[x][y] == ' ')
                    {
                        mCells[x].Add(CellMapType.Void);
                    }
                    else if (mapContext[x][y] == 'д')
                    {
                        mCells[x].Add(CellMapType.DestructiveWall);
                    }
                }
            }

            map.Cells = mCells.ToArray();
