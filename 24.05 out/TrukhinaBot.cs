...
{
    public class TrukhinaBot : IClientBot
    {
        ...
        {
            return (Math.Abs(Request.Tank.Rectangle.LeftCorner.LeftInt - ClosestEnemy.Rectangle.LeftCorner.LeftInt) < 4 &&
                ...
                (Request.Tank.Rectangle.LeftCorner.TopInt > ClosestEnemy.Rectangle.LeftCorner.TopInt && Request.Tank.Direction == DirectionType.Up));
        }

        public bool isInLineY()
        {
            return (Math.Abs(Request.Tank.Rectangle.LeftCorner.TopInt - ClosestEnemy.Rectangle.LeftCorner.TopInt) < 4 &&
                ...
                (Request.Tank.Rectangle.LeftCorner.LeftInt > ClosestEnemy.Rectangle.LeftCorner.LeftInt && Request.Tank.Direction == DirectionType.Left));
        }
        ...
        public ClientCommandType GoToTarget()
        {
            ...
            if (Step.X == Tank.LeftInt + 1 && Step.Y == Tank.TopInt && Request.Tank.Direction != DirectionType.Right && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            {
                return ClientCommandType.TurnRight;
            }
            else if (Step.X == Tank.LeftInt + 1 && Step.Y == Tank.TopInt && Request.Tank.Direction == DirectionType.Right && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            {
                var Clear = false;
                for (var y = (int)Step.Y; y < (int)Step.Y + 5; y++)
                {
                    if (LocationMap[(int)Step.X, y] != -1)
                    {
                        Clear = true;
                    }
                    else
                ...
                else if (LocationMap[(int)Step.X, (int)Step.Y - 1] != -1)
                {
                    return ClientCommandType.TurnUp;
                }
                else if(LocationMap[(int)Step.X, (int)Step.Y + 1] != -1)
            ...
            else if (Step.X == Tank.LeftInt - 1 && Step.Y == Tank.TopInt && Request.Tank.Direction != DirectionType.Left && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            {
                return ClientCommandType.TurnLeft;
            }
            else if (Step.X == Tank.LeftInt - 1 && Step.Y == Tank.TopInt && Request.Tank.Direction == DirectionType.Left && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            ...
            else if (Step.X == Tank.LeftInt && Step.Y == Tank.TopInt + 1 && Request.Tank.Direction != DirectionType.Down && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            {
                return ClientCommandType.TurnDown;
            }
            else if (Step.X == Tank.LeftInt && Step.Y == Tank.TopInt + 1 && Request.Tank.Direction == DirectionType.Down && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            {
                var Clear = false;
                for (var x = (int)Step.X; x > (int)Step.X - 5; x--)
                {
                    if (LocationMap[x, (int)Step.Y] != -1)
                    {
                        Clear = true;
                    }
                    else
                ...
                else if (LocationMap[(int)Step.X - 1, (int)Step.Y] != -1)
                {
                    return ClientCommandType.TurnLeft;
                }
                else if (LocationMap[(int)Step.X + 1, (int)Step.Y] != -1)
            ...
            else if (Step.X == Tank.LeftInt && Step.Y == Tank.TopInt - 1 && Request.Tank.Direction != DirectionType.Up && LocationMap[(int)Step.X, (int)Step.Y] != -1)
            {
                return ClientCommandType.TurnUp;
            }
            else if (Step.X == Tank.LeftInt && Step.Y == Tank.TopInt - 1 && Request.Tank.Direction == DirectionType.Up && LocationMap[(int)Step.X, (int)Step.Y] != -1)
        ...
        public void FindStep()
        {
            ...
            if (LocationMap[Tank.LeftInt, Tank.TopInt - 1] != -1 && ShortWay[Tank.LeftInt, Tank.TopInt - 1] == 1)
            {
            ...
            if (LocationMap[Tank.LeftInt + 1, Tank.TopInt] != -1 && ShortWay[Tank.LeftInt, Tank.TopInt - 1] == 1)
            {
        ...
        public int[,] FindShortestWay()
        {
            ...
            MarkedMap[Request.Tank.Rectangle.LeftCorner.LeftInt, Request.Tank.Rectangle.LeftCorner.TopInt] = 0;
            while (add)
            {
                for (var i = 0; i < MarkedMap.GetLength(0); i++)
                {
